...
int load_p12(gnutls_certificate_credentials_t xcred, char* p12_file, char* password)
{
    ...
    for (i=0; i < MAX_CERTS; i++) { certs[i] = NULL ; }
    int id_cert = -1 ;
    ...
    if (ret < 0) {free(p12blob.data) ; goto error ;}
    free(p12blob.data) ;
        ...
        for (i=0; i < bag_count; i++) {
            ...
            switch(type) {
                ...
                case GNUTLS_BAG_PKCS8_ENCRYPTED_KEY:
                    ...
                    ret = gnutls_x509_privkey_import_pkcs8(key, &data, GNUTLS_X509_FMT_DER,
                          password, type == GNUTLS_BAG_PKCS8_KEY ? GNUTLS_PKCS_PLAIN : 0) ;
                    if (ret < 0) { goto error ; }
